...
void CrowdNavigation::CreateScene()
{
    ...
    crowdManager->SetMaxAgentRadius(0.1f);
    ...
    //SpawnCharacter(Vector3(-25.0f, 0.0f, 20.0f), scene_->CreateChild("characters"))
...
camera->SetFov(75);
    ...
    cameraNode_->SetPosition(Vector3(-20.0f, 50.0f, -20.0f));
...
void CrowdNavigation::SubscribeToEvents()
{
    ...
    SubscribeToEvent(E_MOUSEWHEEL, URHO3D_HANDLER(CrowdNavigation, HandleMouseWheel)) ;
...
void CrowdNavigation::MoveCamera(float timeStep)
{
    ...
    if (!ui->GetCursor()->IsVisible())
    {
        ...
        Urho3D::String foo2(Quaternion(pitch_, yaw_, 0.0f));

        // Construct new orientation for the camera scene node from yaw and pitch. Roll is fixed to zero
        cameraNode_->SetRotation(Quaternion(pitch_, yaw_, 0.0f));
    }

    static Vector3 accel = Vector3(0.0,0.0,0.0);

    // Read WASD keys and move the camera scene node to the corresponding direction if they are pressed
    if (input->GetKeyDown(KEY_W))
        cameraNode_->Translate((cameraNode_->GetWorldDirection() * Vector3(1,0,1)).Normalized() * 2 * MOVE_SPEED * timeStep, TS_WORLD);
    if (input->GetKeyDown(KEY_S))
        cameraNode_->Translate((cameraNode_->GetWorldDirection() * Vector3(-1,0,-1)).Normalized() * 2 * MOVE_SPEED * timeStep, TS_WORLD);
    if (input->GetKeyDown(KEY_A))
        cameraNode_->Translate(Vector3(-2.0f, 0.0f, 0.0f)  * MOVE_SPEED * timeStep);
    if (input->GetKeyDown(KEY_D))
        cameraNode_->Translate(Vector3(2.0f, 0.0f, 0.0f) * MOVE_SPEED * timeStep);

    // Handle zoom
    cameraNode_->Translate(Vector3(0,0, input->GetMouseMoveWheel()) );
...
void CrowdNavigation::HandleMouseWheel(StringHash eventType, VariantMap& eventData)
{
    // Move the camera, scale movement with time step
    //MoveCamera(timeStep);
...
void CrowdNavigation::HandleCrowdAgentFormation(StringHash eventType, VariantMap& eventData)
{
    ...
    if (index)
    {
        ...
        Vector3 position2 = position + Vector3(0, 0, 1.0);

        Urho3D::String foo(position);

        Urho3D::String foo2(position2);

        URHO3D_LOGINFO(foo);

        //URHO3D_LOGINFO(foo2);

        eventData[P_POSITION] = crowdManager->FindNearestPoint(position2, agent->GetQueryFilterType());
    }
}
